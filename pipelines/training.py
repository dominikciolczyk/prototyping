...
def cloud_resource_prediction_training( raw_dir: str,
    ...
    interpolated_dfs = anomaly_detector_and_interpolator(
        dfs=trimmed_dfs,
        scaler_method="standard",  # or "minmax", "robust", "max"
        group_scaling=False,
        z_threshold=3.0,  # typically 2.5–3.5
        interpolation_method="spline",  # "linear", "spline", "polynomial", etc.
        interpolation_order=2,
    )
    plot_time_series(interpolated_dfs, "interpolated_dfs")


    aggregated_dfs = aggregator(interpolated_dfs)
